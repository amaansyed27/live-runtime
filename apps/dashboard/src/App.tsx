...
interface AutomationItem { id: string; title: string; prompt: string; schedule: string; enabled: boolean; }
interface SkillItem { id: string; name: string; status: "Learning" | "Ready" | "Draft"; detail: string; }
...
export function App() {
  ...
  useEffect(() => { void getRuntimeStatus().then(setStatus); try { setWindowLabel(getCurrentWindow().label); } catch { setWindowLabel("main"); } }, []);
    ...
    return (
      <main className="floating-panel">
        <header className="floating-titlebar" data-tauri-drag-region>
          ...
          <div className="titlebar-actions"><button type="button" title="Collapse companion" onClick={() => setCollapsed((value) => !value)}>{collapsed ? "Open" : "—"}</button><button type="button" title="Hide companion" onClick={() => void hideCompanion()}>×</button></div>
        </header>
        {!collapsed && <section className="floating-body"><div className="mini-status"><span>Ready</span><strong>Ask or dictate</strong></div><section className="mini-conversation" aria-label="Recent companion chat">{chat.messages.slice(-4).map((message) => <MessageBubble key={message.id} message={message} />)}</section><ChatComposer disabled={chat.isLoading} onSend={chat.send} /><button type="button" title="Clear this conversation" onClick={chat.clear}>New chat</button></section>}
      </main>
    );
  }
  ...
  return (
    <main className="desktop-frame">
      <header className="custom-titlebar" data-tauri-drag-region>
        ...
        <div className="titlebar-actions"><button type="button" title="Minimize" onClick={() => void getCurrentWindow().minimize()}>—</button><button type="button" title="Hide to tray" onClick={() => void hideToTray()}>×</button></div>
      </header>
...
function Brand({ label, compact = false }: { label: string; compact?: boolean }) { return <div className={`brand-block ${compact ? "brand-compact" : ""}`} data-tauri-drag-region><img className="brand-mark" src="/live-runtime-logo.svg" alt="Live Runtime logo" /><div><p>{label}</p><small>Local AI</small></div></div>; }
function Tip({ text }: { text: string }) { return <span className="tip" title={text}>?</span>; }

function ChatPage({ chat }: { chat: ReturnType<typeof useRuntimeChat> }) { return <section className="page-panel chat-page"><div className="page-hero"><p className="eyebrow">Local AI</p><h1>Ask anything.</h1><span>Saved until New chat.</span></div>{chat.error && <div className="error-banner">Failed to connect. Is Ollama running?</div>}<section className="conversation" aria-label="Conversation">{chat.messages.map((message) => <MessageBubble key={message.id} message={message} />)}</section><ChatComposer disabled={chat.isLoading} onSend={chat.send} /></section>; }
