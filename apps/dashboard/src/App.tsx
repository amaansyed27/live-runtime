...
type Page = "chat" | "settings" | "automation" | "skills";
...
export function App() {
  ...
  useEffect(() => {
    ...
    if (windowLabel === "main") void (companionEnabled ? showCompanion() : hideCompanion());
  ...
  return (
    <main className="desktop-frame">
      ...
      <section className="app-body">
        <nav className="page-rail page-rail-four" aria-label="Main sections">
          ...
          <button className={page === "skills" ? "active" : ""} type="button" title="Learning and skills" onClick={() => setPage("skills")}>Skills</button>
        </nav>
...
function SettingsPage({ chat, status, shortcut, setShortcut, companionEnabled, setCompanionEnabled, theme, setTheme, startAtLogin, setStartAtLogin, autoStartOllama, setAutoStartOllama }: {
  ...
  return (
    <section className="page-panel settings-page">
      ...
      <div className="settings-grid">
        ...
        <section className="settings-card toggle-card"><label title="Ollama supervisor is the next native layer"><input type="checkbox" checked={autoStartOllama} onChange={(event) => setAutoStartOllama(event.target.checked)} />Start Ollama</label></section>
        <section className="settings-card toggle-card"><label><input type="checkbox" checked={chat.speakResponses} onChange={(event) => chat.setSpeakResponses(event.target.checked)} />Voice replies</label><button type="button" title="Stop current speech" onClick={() => void stopSpeech()}>Stop</button></section>
        <section className="settings-card"><div className="status-grid"><SignalCard label="Host" value={status.platform} tone="good" /><SignalCard label="Arch" value={status.arch} /><SignalCard label="Speech" value={status.speechOutput} tone="good" /><SignalCard label="Tray" value={status.trayEnabled ? "on" : "web"} /></div></section>
      </div>
    </section>
  );
}

function AutomationPage({ automations, setAutomations, draft, setDraft }: {
  ...
  const createAutomation = () => {
    if (!canCreate) return;
    setAutomations((current) => [{ id: crypto.randomUUID(), title: draft.title.trim(), prompt: draft.prompt.trim(), schedule: draft.schedule.trim(), enabled: true }, ...current]);
    setDraft({ title: "", prompt: "", schedule: "" });
  };

  return (
    <section className="page-panel automation-page">
      <div className="page-header"><p className="eyebrow">Routines</p><h2>Scheduled prompts</h2></div>
      <div className="automation-layout routine-layout">
        <section className="automation-card automation-builder routine-builder">
          <div className="section-title-row"><span>Create</span><small>Like Codex automations</small></div>
          ...
          <textarea placeholder="Prompt to run" title="Prompt to run" value={draft.prompt} onChange={(event) => setDraft({ ...draft, prompt: event.target.value })} rows={3} />
          <input placeholder="Schedule" title="Schedule text" value={draft.schedule} onChange={(event) => setDraft({ ...draft, schedule: event.target.value })} />
          <button type="button" title="Save routine" disabled={!canCreate} onClick={createAutomation}>Create</button>
        </section>

        <section className="automation-card template-card">
          <span>Templates</span>
          <div className="template-list">{automationExamples.map((example) => <button key={example.title} type="button" title={`Use ${example.title}`} onClick={() => setDraft(example)}>{example.title}</button>)}</div>
        </section>

        <section className="automation-list routine-list">
          {automations.length === 0 && <div className="empty-state compact-empty"><strong>No routines yet.</strong><p>Pick a template or create one.</p></div>}
          {automations.map((automation) => (
            <article className="automation-item" key={automation.id}>
              <div><span>{automation.enabled ? "On" : "Paused"}</span><h3>{automation.title}</h3><p>{automation.prompt}</p><small>{automation.schedule}</small></div>
              <div className="automation-actions"><button type="button" title="Pause or enable routine" onClick={() => setAutomations((current) => current.map((item) => item.id === automation.id ? { ...item, enabled: !item.enabled } : item))}>{automation.enabled ? "Pause" : "Enable"}</button><button type="button" title="Delete routine" onClick={() => setAutomations((current) => current.filter((item) => item.id !== automation.id))}>Delete</button></div>
            </article>
          ))}
        </section>
      </div>
...
function SkillsPage({ skills, setSkills, searchProvider, setSearchProvider }: {
  skills: SkillItem[];
  setSkills(value: SkillItem[] | ((current: SkillItem[]) => SkillItem[])): void;
  searchProvider: string;
  setSearchProvider(value: string): void;
}) {
  const resetSkills = () => setSkills(defaultSkills);

  return (
    <section className="page-panel skills-page">
      <div className="page-header"><p className="eyebrow">Skills</p><h2>Learning memory</h2></div>
      <div className="skills-layout">
        <section className="skill-hero">
          <span>Growth model</span>
          <strong>Observe → Save → Reuse</strong>
        </section>

        <section className="settings-card">
          <label htmlFor="searchProvider">Search provider <Tip text="Use your own SearXNG instance for reliable free JSON search." /></label>
          <input id="searchProvider" value={searchProvider} onChange={(event) => setSearchProvider(event.target.value)} title="SearXNG instance URL" />
        </section>
        ...
        <section className="automation-card compact-note">
          <span>skills.md</span>
          <strong>Local skill file comes next.</strong>
          <p>The companion will promote repeated patterns into reusable skills.</p>
        </section>

        <button type="button" title="Restore starter skills" onClick={resetSkills}>Reset starter skills</button>
      </div>
    </section>
  );
}
